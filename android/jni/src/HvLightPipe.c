...
char *hLp_getWriteBuffer(HvLightPipe *q, const uint32_t bytesToWrite) {
  ...
  if (totalByteRequirement <= q->remainingBytes) {
    char *const newWriteHead = oldWriteHead + sizeof(uint32_t) + bytesToWrite;
    ...
    if (totalByteRequirement <= q->len) {
      if ((oldWriteHead < q->readHead) || ((q->buffer + totalByteRequirement) > q->readHead)) {
...
char *hLp_getReadBuffer(HvLightPipe *q, uint32_t *numBytes) {
  *numBytes = HLP_GET_UINT32_AT_BUFFER(q->readHead);
  char *const readBuffer = q->readHead + sizeof(uint32_t);
  return readBuffer;
}
